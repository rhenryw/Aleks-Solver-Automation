...
class ChatbotSolver:
    ...
    def _ask_gemini(self, prompt: str, image_path: Path | None = None) -> str:
        ...
        # ── Force React to register input & Send ──────────────────────────
        p.keyboard.press("Space")
        time.sleep(0.1)
        p.keyboard.press("Backspace")
        time.sleep(0.1)
        
        # Capture baseline BEFORE sending so the new response is never counted early
        baseline = p.evaluate(
            "() => document.querySelectorAll('model-response, .model-response').length"
        )

        # In Gemini, just pressing Enter always submits the prompt
        p.keyboard.press("Enter")
        log.info("  Gemini: sent ✓")
    ...
    def _extract_answer(self, raw: str) -> str:
        """
        Parse the chatbot's raw response into the structured ALEKS answer.

        Strategy (mirrors auto-mcgraw's JSON extraction):
          1. Strip zero-width unicode chars
          2. JSON fenced code block  ```json { } ```
          3. Bare JSON object { }
          4. Nested JSON scan (greedy outer match)
          5. \\boxed{} fallback
          6. Last non-empty line
        """
        text = raw.strip()

        # 1. Clean zero-width chars (auto-mcgraw does this too)
        text = re.sub(r'[\u200b\u200c\u200d\ufeff]', '', text)
        ...
        # 4. Any JSON object anywhere in response
        for m in re.finditer(r'\{[^{}]*(?:\{[^{}]*\}[^{}]*)?\}', text, re.DOTALL):
            try:
                data = json.loads(m.group(0))
                if isinstance(data, dict) and (
                    "type" in data or "answer" in data
                ):
    ...
    def solve_from_screenshot(
        ...
        """
        Solve an ALEKS question using only the screenshot and a simple prompt.
        """
        if not image_path.exists():
            log.warning(f"  Screenshot not found: {image_path}")
            return "", None

        question_text = "Please provide the final answer for the attached image."
        answer = self.solve(question_text, context=context, image_path=image_path)
        return answer, None
